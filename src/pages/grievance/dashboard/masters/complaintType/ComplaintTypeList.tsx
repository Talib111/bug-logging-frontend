...
export default function ComplaintTypeList() {
  ...
  const complaintSourceData = useApi<I_COMPLAINT_SOURCE_TYPE_LIST>({
    api: `${grievanceAPI.getAllComplaintType}?page=${page}&limit=${perPage}&q=${search}`,
  ...
  const handelStatusUpdate = async (Id: string) => {
    try {
      const result = await mutate.mutateAsync({
        api: `${grievanceAPI?.updateComplaintTypeStatus}/${Id}`,
        data: {
          id: id
        }
      });
  ...
  return (
    <main className='grid items-start'>
      <ComplaintTypeForm
        open={open}
        setOpen={setOpen}
        title={edit ? 'Edit Grievance Type' : 'Add Grievance Type'}
        id={id}
        edit={edit}
        setEdit={setEdit}
      ...
      <div className='grid auto-rows-max items-start gap-4 md:gap-2 lg:col-span-2'>
        ...
        <Card >
          <CardHeader className='px-7'>
            <CardDescription>
            Complaint Type List ({complaintSourceData.data?.data?.totalDocs})
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className=''> Grievance Type Name</TableHead>
                  ...
                  <TableBody>
                    {complaintSourceData?.data?.data?.docs?.map((items, index) => (
                      <TableRow key={items?._id}>
                        ...
                        <TableCell>{items?.complaintTypeName}</TableCell>
