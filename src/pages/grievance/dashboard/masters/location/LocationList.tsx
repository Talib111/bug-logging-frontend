...
import { I_LOCATION_TYPE_LIST } from './type'
...
export default function HomePage() {
  ...
  const complaintData = useApi<I_LOCATION_TYPE_LIST>({
    api: `${grievanceAPI.getAllLocation}?page=${page}&limit=${perPage}&q=${search}`,
    key: 'getAllLocation',
    value: [page, perPage],
    options: {
      enabled: true,
  ...
  const handelStatusUpdate = async (Id: string) => {
    try {
      const result = await mutate.mutateAsync({
        api: `${grievanceAPI?.updateLocationStatus}/${Id}`,
        data: {
          id: id
        }
      });
      ...
      <div className='grid auto-rows-max items-start gap-4 md:gap-2 lg:col-span-2'>
        ...
        <Card>
          <CardHeader className='px-7'>
            <CardDescription>
            Location List ({complaintData.data?.data?.totalDocs})
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className=''>Radius(In Meter)</TableHead>
                  ...
                  <TableBody>
                    {complaintData?.data?.data?.docs?.map((items, index) => (
                      <TableRow key={items?._id}>
                        ...
                        <TableCell>{items?.radius}</TableCell>
